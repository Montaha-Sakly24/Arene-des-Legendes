class UI {
  ...
  verifyElements() {
    ...
    requiredElements.forEach((elementName) => {
      if (!this[elementName]) {
        console.warn(` Élément UI manquant: ${elementName}`);
      }
    });
  ...
  selectPlayerCount(btn) {
    ...
    console.log(` Nombre de joueurs sélectionné: ${this.selectedPlayerCount}`);
  ...
  showDiceOrderSection(playerName, isTieBreaker = false) {
    ...
    if (alivePlayers.length <= 1) {
      // S'il ne reste qu'un joueur ou moins, ne pas montrer la section des dés
      window.game.endGame();
      return;
    }
  ...
  updateDiceResults(results) {
    ...
    // Ne montrer que les résultats des joueurs vivants
    ...
    console.log(
      ` Résultats mis à jour: ${aliveResults.length} joueurs vivants`
    );
  }
  ...
  showFinalOrder(results, callback) {
    ...
    console.log(" Ordre final affiché (joueurs vivants uniquement)");
  }
  ...
  createArenaGrid() {
    ...
    console.log(" Grille d'arène créée (7x7)");
  }
  ...
  showMessage(title, text, callback = null) {
    ...
    // Pour les autres messages (attaque/pouvoir spécial)
    if (
      title.toLowerCase().includes("pouvoir spécial") ||
    ...
    // Pour tous les autres messages, juste logger et exécuter le callback
